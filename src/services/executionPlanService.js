...
function geometryFor(segment){
  const geometry=segment.geometry?.length>=2?segment.geometry:[segment.origin,segment.destination];
  return geometry.filter(Boolean);
}
function pointOnGeometry(geometry,progress){
  if(geometry.length===1)return geometry[0];
  const portions=geometry.slice(1).map((point,index)=>Math.hypot(
    point.latitude-geometry[index].latitude,point.longitude-geometry[index].longitude
  ));
  ...
  for(let index=0;index<portions.length;index++){
    if(remaining<=portions[index]||index===portions.length-1){
      return linePoint(geometry[index],geometry[index+1],portions[index]?remaining/portions[index]:0);
    }
...
function geometryUntil(geometry,progress){
  ...
  for(let index=0;index<portions.length;index++){
    if(remaining>=portions[index]){
      ...
      path.push(linePoint(geometry[index],geometry[index+1],portions[index]?remaining/portions[index]:0));
      break;
    }
  }
  return path;
...
function appendPath(target,points){
  for(const point of points)if(!samePoint(target.at(-1),point))target.push(point);
  return target;
}
function buildSegments(plan,result){
  const points=Object.fromEntries([result.origin,result.destination,...(plan.facilities||[])].map(point=>[point.name,point]));
  return plan.segments
    .filter(segment=>segment.distanceKm>0)
    .map((segment,index)=>({
      index,from:segment.from,to:segment.to,modal:segment.modal,
      distanceKm:segment.distanceKm,timeMin:segment.timeMin,
      origin:segment.origin||points[segment.from],
      destination:segment.destination||points[segment.to],
      geometry:segment.geometry,progress:0,status:index?'PENDENTE':'EM_ANDAMENTO'
    }))
    .filter(segment=>segment.origin&&segment.destination);
}
function snapshot(state){
  if(!state)return null;
  const current=state.segments[state.currentSegmentIndex]||state.segments.at(-1);
  const currentGeometry=current?geometryFor(current):[];
  const currentPosition=current?pointOnGeometry(currentGeometry,current.progress):state.destination;
  const remainingPlanDistance=state.segments.reduce((sum,segment)=>sum+segment.distanceKm,0);
  const currentPlanTraveled=state.segments.reduce((sum,segment)=>sum+segment.distanceKm*segment.progress,0);
  ...
  const remainingPath=state.segments.reduce((path,segment)=>appendPath(path,geometryFor(segment)),[]);
  const currentPath=[];
  for(const segment of state.segments){
    appendPath(currentPath,geometryUntil(geometryFor(segment),segment.progress));
  ...
  return JSON.parse(JSON.stringify({
    ...state,currentSegment:current?{...current,position:currentPosition}:null,currentPosition,
    totalPath,traveledPath,totalDistanceKm,traveledKm,remainingKm,
    ...
    remainingMarginMinutes:state.maximumIschemiaMinutes-state.initialConsumedMinutes-elapsedMinutes
  }));
}
function freeze(id,plan,result){
  const state={
    transporteId:Number(id),planId:plan.id,planName:plan.name,modal:plan.modal,
    origin:result.origin,destination:result.destination,segments:buildSegments(plan,result),
    currentSegmentIndex:0,simulationElapsedSeconds:0,historicalPath:[],historicalDistanceKm:0,
    ...
    operationalSafetyMarginMinutes:result.profile.ischemia.operationalSafetyMarginMinutes
  };
...
function replace(id,plan,result){
  ...
  if(!state)throw Object.assign(new Error('Não há execução ativa para reotimizar.'),{status:409});
...
function advance(id,realElapsedSeconds){
  const state=plans.get(Number(id));
  if(!state)return null;
  const delta=Math.max(0,Number(realElapsedSeconds)||0)*config.simulationTimeScale;
  ...
  while(remaining>0&&state.segments[state.currentSegmentIndex]){
    ...
    if(remaining>=needed){
      segment.progress=1;segment.status='CONCLUIDO';remaining-=needed;state.currentSegmentIndex++;
      if(state.segments[state.currentSegmentIndex])state.segments[state.currentSegmentIndex].status='EM_ANDAMENTO';
    }else{
...
function finish(id){
  const state=plans.get(Number(id));
  if(!state)return null;
  state.segments.forEach(segment=>{segment.progress=1;segment.status='CONCLUIDO'});
  state.currentSegmentIndex=state.segments.length;
  return snapshot(state);
}
module.exports={freeze,replace,advance,finish,get:id=>snapshot(plans.get(Number(id))),reset:id=>plans.delete(Number(id)),timeScale:()=>config.simulationTimeScale};