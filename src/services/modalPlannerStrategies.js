...
async function bestGroundSegment(from, to, conditions, phase) {
  ...
  const selected = [...routes].sort(
    (a, b) => a.cost - b.cost || a.timeMin - b.timeMin || a.risk - b.risk,
  )[0];
  return segment(
    from.name,
    to.name,
...
function helicopterAccessSegment(from, to, phase) {
  ...
  return segment(
    ...
    "HELICÓPTERO",
    distanceKm,
    (distanceKm / helicopter.averageSpeedKmh) * 60 +
      helicopter.activationTimeMin,
    helicopter.baseCost + distanceKm * helicopter.costPerKm,
    {
      ...
      classification: "SIMULATED_ASSUMPTION",
      source:
        "Acesso aéreo demonstrativo entre hospital e aeroporto; custo e tempo simulados.",
    },
  );
}
class ModalPlannerStrategy {
  ...
  travelTime(distance) {
    return (distance / this.config.averageSpeedKmh) * 60;
  }
...
class HelicopterTransportStrategy extends ModalPlannerStrategy {
  ...
  async plan({ origin, destination, locationProvider, conditions }) {
    const [departure, arrival] = await Promise.all([
      conditions.facilities?.HELIPORT_ORIGIN ||
        locationProvider.findFacility(origin, "HELIPORT_ORIGIN"),
      conditions.facilities?.HELIPORT_DESTINATION ||
        locationProvider.findFacility(destination, "HELIPORT_DESTINATION"),
    ]);
    const hasOrigin = conditions.originHasHelipad,
      hasDestination = conditions.destinationHasHelipad;
    const legs = [];
    if (!hasOrigin)
      legs.push(
        await bestGroundSegment(origin, departure, conditions, "origem"),
      );
    const airDistance =
      haversine(
    ...
    legs.push(
      this.operationalSegment(
        "Acionamento e preparação",
        this.config.activationTimeMin + this.config.preparationTimeMin,
        this.config.baseCost,
      ),
    );
    legs.push(
      segment(
        hasOrigin ? origin.name : departure.name,
        hasDestination ? destination.name : arrival.name,
        "HELICÓPTERO",
        airDistance,
...
class MultimodalAirTransportStrategy extends ModalPlannerStrategy {
  ...
  async plan({ origin, destination, locationProvider, conditions }) {
    const [departure, arrival] = await Promise.all([
      conditions.facilities?.AIRPORT_ORIGIN ||
        locationProvider.findFacility(origin, "AIRPORT_ORIGIN"),
      conditions.facilities?.AIRPORT_DESTINATION ||
        locationProvider.findFacility(destination, "AIRPORT_DESTINATION"),
    ]);
    const accessOptions = [
      {
        key: "T",
        label: "Terrestre",
        available: conditions.groundAccessOriginAvailable !== false,
        segment: () =>
          bestGroundSegment(
            origin,
            departure,
            conditions,
            "acesso ao aeroporto de origem",
          ),
      },
      {
        key: "H",
        label: "Helicóptero",
        available: Boolean(
          conditions.originHasHelipad &&
          conditions.infrastructureAvailability.HELIPORT_ORIGIN,
        ),
        segment: async () =>
          helicopterAccessSegment(
            origin,
            departure,
            "acesso ao aeroporto de origem",
          ),
    ...
    const exitOptions = [
      {
        key: "T",
        label: "Terrestre",
        available: conditions.groundAccessDestinationAvailable !== false,
        segment: () =>
          bestGroundSegment(
            arrival,
            destination,
            conditions,
            "saída do aeroporto de destino",
          ),
      },
      {
        key: "H",
        label: "Helicóptero",
        available: Boolean(
          conditions.destinationHasHelipad &&
          conditions.infrastructureAvailability.HELIPORT_DESTINATION,
        ),
        segment: async () =>
          helicopterAccessSegment(
            arrival,
    ...
    const flight = segment(
      ...
      "AVIÃO",
      flightDistance,
      this.travelTime(flightDistance),
      flightDistance * this.config.costPerKm * (1 + conditions.airCostIncrease),
      {
        phase: "voo entre aeroportos",
        geometry: [departure, arrival],
        classification: "SIMULATED_ASSUMPTION",
        source:
          "Voo demonstrativo entre aeroportos reais identificados por ICAO.",
      },
    );
    const preparation = this.operationalSegment(
      "Preparação, embarque e transferência aeroportuária",
      this.config.activationTimeMin +
        this.config.preparationTimeMin +
        this.config.transferTimeMin,
      this.config.baseCost,
    );
    const plans = [];
...
function strategies() {
  return [
    new GroundTransportStrategy(),
    new HelicopterTransportStrategy(),
    new MultimodalAirTransportStrategy(),
  ];
}
module.exports = {
  ModalPlannerStrategy,
