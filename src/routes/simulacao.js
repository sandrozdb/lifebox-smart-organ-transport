...
router.post('/start',async(req,res,next)=>{try{res.json(await service.start(req.body.transporteId||1,req.body.rotaId))}catch(e){next(e)}});
router.post('/stop',async(_req,res,next)=>{try{res.json(await service.stop())}catch(e){next(e)}});
router.post('/reset',async(req,res,next)=>{try{res.json(await service.reset(req.body.transporteId||1))}catch(e){next(e)}});
