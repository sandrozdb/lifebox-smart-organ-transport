(() => {
  ...
  const api = async (path, options = {}) => {
    ...
    if (!response.ok) throw Error(data.error || data.erro || "Falha");
    return data;
  };
  const money = (value) =>
  ...
  const reasonLabel = (key) =>
    ({
      ...
      traffic30: "Aumento de 30% no tempo terrestre",
  ...
  function render(result, reason) {
    ...
    if (
      ...
      res.insertAdjacentHTML(
        "beforeend",
        reoptimizationMarkup(window.lifeBoxReoptimizationRecommendation),
      );
  ...
  async function applyRecommendation() {
    ...
    try {
      await api("/api/simulacao/reotimizar/aplicar", {
        method: "POST",
        body: JSON.stringify({
          transporteId: window.lifeBoxTransportId,
      ...
      $("#planning-result").innerHTML =
        `<strong>REOTIMIZAÇÃO APLICADA</strong><span>${recommendation.plan.modal}</span><p>O plano ativo foi atualizado a partir da posição atual. Tempo, isquemia e caminho já percorrido foram preservados.</p>`;
